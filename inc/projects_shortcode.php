<?php
    class Unikassel_Projects{
        ...
        public static function get_fist_step(){
            return self::$fist_step;
        }
        ...
        public function render_edit(){
            ...
            if( get_post_type() == 'project' && empty($this->project_factors()) ){
                ...
                $link = $this->dashboard_url().'/?edit='.$this->success_factors.'&project_ref='.get_the_id().'&step_ref='. $this->curr_step();
                printf(
        ...
        public function report_data(){
            ...
            $data['orgnisation']['profits_for_the_organization'] = get_post_meta($id, 'profits_for_the_organization'.'_'. $key , true);
            $data['orgnisation']['tasks_for_the_organization'] = get_post_meta($id, 'tasks_for_the_organization'.'_'. $key , true);

            $data['team']['profits_for_the_team'] = get_post_meta($id, 'profits_for_the_team'.'_'. $key , true);
            $data['team']['team_preparation'] = get_post_meta($id, 'team_preparation'.'_'. $key , true);

            $data['process']['neue__angepsste_prozesse'] = get_post_meta($id, 'neue__angepsste_prozesse'.'_'. $key , true);

            $data['contractual']['new_agreements'] = get_post_meta($id, 'new_agreements'.'_'. $key , true);

            $data['knowladage']['new__angepsste_communication_channels'] = get_post_meta($id, 'new__angepsste_communication_channels'.'_'. $key , true);

            $data['Projektdokumentationen']['table'] = $this->for_flix_results();
        ...
        public function step_args($step){
            ...
            $args['html_submit_button'] = '<input type="submit" class="acf-button button button-primary button-large" value="'.__('Save Enabler','uni').'" />'.$this->factor_save_btn();
            return $args;
        }
        public function is_enabler_completed(){
            $id = get_the_ID();
            $results = $this->for_repaeter_results();
            foreach ($results as $result) {
                if (!isset($result['d']) || $result['d'] !== 'checked') {
                    return false;
                }
            }
            return true;
        }
        public function factor_save_btn(){
            $data['done'] =  $this->is_enabler_completed();
            ob_start();
            get_template_part('template-parts/project', 'complete-btn' ,$data);
            $html = ob_get_contents();
            ob_end_clean();
            return $html;
        }

        public function for_repaeter_results(){
            $factor_key = $this->factor_key($_GET['step']);
            $id = get_the_ID();
            $cond = " `post_id` =   $id " . " AND  `meta_key`  LIKE 'requirements_for_".$factor_key."_%' ";
            $results = $this->get_reapeter($cond);
            $results = $this->collect_reqs_for_repaeter($results , false);
            return $results;
        }

        public function for_flix_results(){
            $factor_key = $this->factor_key($_GET['report']);
            $id = get_the_ID();
            $cond = " `post_id` =   $id " . " AND  `meta_key`  LIKE 'Projektdokumentationen_table_".$factor_key."_%' ";
            $results = $this->get_reapeter($cond);
            $results = $this->collect_rows_for_flex($results);
            return $results;
        }

        public static function collect_rows_for_flex($results = array()){
            $collect = array();
            foreach ($results as $result) {
                $keys =  explode('_', $result->meta_key);
                    $id = $keys[3];
                    if(@$keys[4] == 'anderungen'){
                        $collect[$id]['a'] = $result->meta_value;
                    }elseif (@$keys[4] == 'kommunikation') {
                        $collect[$id]['k'] = $result->meta_value;
                    }elseif (@$keys[4] == 'title') {
                        $collect[$id]['t'] = $result->meta_value;
                    }elseif (@$keys[4] == 'person') {
                        $collect[$id]['p'] = $result->meta_value;
                    }
            }
            return $collect;
        }

        public function factor_key($info,$type='name'){
            $info = $type == 'name' ? $this->str_key($info) : $info;
            $success_factors =  get_field('success_factors', 'option');
            foreach ( $success_factors as $factor) {
                if ( $info ==  $this->str_key($factor['name'])  ) {
                    return $factor['key'];
                }
            }
        }
        ...
        public static function project_factors($id = 0){
            ...
            foreach ($success_factors as $key => $factor) {
                //if(!is_array($factor['projects'])   || ( isset($_GET['sucess_factor_1']) && isset($_GET['sucess_factor_1']) && !in_array($id,$factor['projects']) ) ){
                if(isset($factor['projects']) &&   !is_array($factor['projects'])  ||     (  (get_post_type() == 'page' || ( get_post_type() == 'project' && isset($_GET['step']))  ) &&  !in_array($id,$factor['projects'])  )        ){
        ...
        public static function enablers(){
            return array(
                'organization_management'   => array('Organization',1),
                'team_management'           => array('Team',1),
                'process_management'        => array('Process Management',1),
                'contractual_management'    => array('Contractual Management Network',1),
                'knowladage_management'     => array('Knowladage Management',1),
                'projektdokumentationen'    => array('Projektdokumentationen',0),
            );
        }
        ...
        public static function collect_reqs_for_repaeter($results = array(),$filter=true){
            $collect = array();
            ...
            foreach ($collect as $key => $item) {  
                if(!isset($item['n']) || trim($item['n']) == '' || ($filter == true &&  $item['e'] != $_GET['enabler']) ){
                    unset($collect[$key]);
                }else{
